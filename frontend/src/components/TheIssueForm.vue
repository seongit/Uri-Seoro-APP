                  ...
                  <div class="col-sm-5">
                    ...
                    <div class="row">
                      <!--첨부파일 업로드 시 반복적으로 생성되는 영역-->
                        <div class="col-sm-11" v-for="(item,index) in uploadFileList"
                        :key="index"
                        :value="item.id">
...
export default {
  ...
  methods : {
    ...
    settingSelectBoxList(){
      ...
      // 유형 전체 목록 조회 
      apiIssue.getTrackerList().then((response)=>{
      ...
      // 상태 전체 목록 조회
      apiIssue.getIssueStatusArr().then((response) => {
      ...
      // 우선 순위 전체 목록 조회
      apiIssue.getPrioritiesArr().then((response) => {
    ...
    setSelect(event){
      ...
      switch(divId){
        case "selectBoxProject" : {this.projectId = value ;} break;
        case "selectBoxTracker" : {this.trackerId = value; console.log(value);} break;
        case "selectBoxPriority" : {this.priorityId = value;} break;
        case "selectBoxStatus" : {this.statusId = value;} break;
        case "selectBoxDoneRatio" : {this.doneRatio = value;} break;
    ...
    handleFileChange(event){
      ...
      apiIssue.postFileUploadCreateToken(formData).then((response)=>{
